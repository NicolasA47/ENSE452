...
void USART2_IRQHandler(void)
{
    ...
    // Check if the reason for the interrupt is because of RXNE.
    if (USART2->SR & USART_SR_RXNE)
    {
        // Read the data register to fetch the received byte and clear the RXNE flag.
        characterReceived = (uint8_t)USART2->DR;

        // Send the received byte to the queue.
        xQueueSendToFrontFromISR(xQueue, &characterReceived, &xHigherPriorityTaskWoken);
    }

    // If a higher priority task was woken by the ISR, request a context switch.
